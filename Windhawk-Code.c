...
// @version         4.0
// @author          SuperStark + Hashah2311
// @github          https://github.com/SuperStark & https://github.com/Hashah2311
...
- AutoTheme: true
  $name: Automatic Theme Colors
- TextColor: ""
  $name: Manual Text Color (Hex, e.g. #FF0000)
  $description: Leave empty to use default white/black.
- BgOpacity: 0
  $name: Background Tint Opacity (0-255)
  $description: Only used if Auto Theme is OFF.
- ShowBorder: false
...
void LoadSettings() {
    ...
    // Load new settings
    g_Settings.showBorder = Wh_GetIntSetting(L"ShowBorder") != 0;
    g_Settings.transparentBg = Wh_GetIntSetting(L"TransparentBackground") != 0;
}
...
void UpdateAppearance(HWND hwnd) {
    ...
    // We only want rounding if we aren't fully transparent
    // Fix: Changed DWMWCP_DONT_ROUND to DWMWCP_DONOTROUND
    DWM_WINDOW_CORNER_PREFERENCE preference = g_Settings.transparentBg ? DWMWCP_DONOTROUND : DWMWCP_ROUND;
    ...
    // 4. Background (Acrylic vs Transparent)
    ...
    if (hUser) {
        ...
        if (SetComp) {
            ...
            if (g_Settings.transparentBg) {
                // Fully Transparent (No Blur)
                policy.AccentState = ACCENT_ENABLE_TRANSPARENTGRADIENT; 
                // Fix: Use 0x00FFFFFF (Fully transparent White). 
                // If we pass pure 0, DWM sometimes glitches to Magenta/Pink.
                policy.GradientColor = 0x00FFFFFF; 
            } else {
                // Acrylic Blur (Standard)
                DWORD tint = 0;
